...
export const TablePage = () => {
   ...
   const checkChanged = async () => {
      try {
         if (
            (await changed(tableId, [playerOneDeck.length, playerTwoDeck.length, playerThreeDeck.length])) !== "false"
         ) {
            const [
               player_one,
               player_two,
   ...
   const handleWinner = () => {
      if (winner === playerOne?.id) return playerOne?.name;
      if (winner === playerTwo?.id) return playerTwo?.name;
      if (winner === playerThree?.id) return playerThree?.name;
   }
   ...
   return (
      <div className="table">
            ...
            {playerId === playerThree?.id &&
               playerThreeDeck.map((card) => (
                  <Card
                     ...
                     onClick={() => handleCardClick(playerId, card)}
                  />
               ))}
            {playerId !== playerThree?.id &&
         ...
         <div className="deck deck_player1 animate__animated animate__slideInUp">
            ...
            {playerId !== playerOne?.id &&
               playerOneDeck.map((card) => <Card key={card?.id} onClick={() => handleCardClick(playerId, card)} />)}
         </div>
